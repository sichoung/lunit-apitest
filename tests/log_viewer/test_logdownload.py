...
def get_logfile_idlist(get_lv_baseurl, prepare_testuser_gettoken):
    search_pagesize = 3
...
def test_logdownload_basic(get_lv_baseurl, prepare_testuser_gettoken, get_logfile_idlist):
    """ 테스트 목적 : 최근 3건의 id 조회 후 다운로드 기본 테스트
    """
    headers = {"Content-Type": "application/json", "Authorization": "Bearer {}".format(prepare_testuser_gettoken(test_email, test_pw))}
    payload = {
...
def test_logdownload_notexistlogid(get_lv_baseurl, prepare_testuser_gettoken):
...
def test_logdownload_notexistlogid2(get_lv_baseurl, prepare_testuser_gettoken, get_logfile_idlist):
