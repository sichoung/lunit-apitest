...
def test_uploadlog_basic(get_lv_baseurl,get_lv_token,get_dirpath):
    ...
    """
    get_lv_token = get_lv_token(url_manager.test_email, url_manager.test_pw)
    ...
    response = requests.post(get_lv_baseurl + url_manager.logupload_api_path, files=payload, headers=headers, verify=False) #data=payload, 
    print(response.text)
