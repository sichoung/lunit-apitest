...
def test_getloglist_basic(get_lv_baseurl,get_lv_token):
    headers = {"Authorization": "Bearer {}".format(get_lv_token(test_id, test_pw))}
    params = {
    ...
    response = requests.get(get_lv_baseurl + APIInfo.getloglist_api_path, headers=headers, verify=False)
