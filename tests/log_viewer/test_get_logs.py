...
def test_get_logs_default_200(lv_api_url, get_lv_token):
...
def test_get_logs_search_200(lv_api_url, get_lv_token):
...
def test_get_logs_nodata(lv_api_url, get_lv_token):
...
def test_get_logs_sort(lv_api_url, get_lv_token):
    """ 테스트 목적 : 정렬 적용을 확인 - 아직 테스트 방법 파악 중
    """
    print("")

def test_get_logs_paging(lv_api_url, get_lv_token):
...
def test_get_logs_search_capitalkeyword(lv_api_url, get_lv_token):
...
def test_get_logs_daterangesearch(lv_api_url, get_lv_token):
    """ 테스트 목적 : 날짜 범위로 검색 
    """
...
def test_get_logs_invalid_dateformat(lv_api_url, get_lv_token):
...
def test_get_logs_wrongcomponenttype(lv_api_url, get_lv_token):
    """ 테스트 목적 : invalid format - 날짜 형태에 2020-09-15T 입력했을 때 응답 확인
    """
...
def test_get_logs_wrongloglevel(lv_api_url, get_lv_token):
...
def test_get_logs_wronglogtype(lv_api_url, get_lv_token):
...
def test_get_logs_notauth(lv_api_url, get_dirpath):
    """ 테스트 목적 : 인증토큰 없이 로그 조회 시도 
    """
    # headers = {"Authorization": "Bearer {}".format(get_lv_token(test_email, test_pw))}
    response = requests.get(lv_api_url + url_manager.getloglist_api_path, verify = False) # = get_dirpath+'/base64_lunit_cert.cer')
